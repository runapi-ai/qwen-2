...
- For a one-off artifact or result, use the registered `qwen-2` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi qwen-2 --help
runapi qwen-2 <operation> --help
curl --fail --location https://runapi.ai/docs/api/qwen-2/<operation>.md --output contract.md
```
...
```shell
runapi qwen-2 <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service qwen-2 --action <operation> > result.json
```
...
- SDK integration: https://github.com/runapi-ai/qwen-2-sdk

## Variants
- `qwen-2-edit-image`: https://runapi.ai/models/qwen-2/edit-image.md
- `qwen-2-text-to-image`: https://runapi.ai/models/qwen-2/text-to-image.md
